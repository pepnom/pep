...
set mm(work) "";       # clear
append mm(work) "> Searching for palindromes and sub-palindromes \n"
append mm(work) "> in the input. Strings of the same character are not \n"
append mm(work) "> considered as palindromes (eg: xxxx or yyy). \n"
append mm(work) "> Whitespace and non-letters are ignored!.\n"
append mm(work) " Palidrome: text that is the same backwards.  \n"
append mm(work) "  παλίν (palin) = back \n"
append mm(work) "  δρομος (dromos) = running \n"
...
while !$mm(eof) { 
  ...
  while true {  
    ...
    Pop;
    Pop;
    if {$mm(work) eq "pal*pal*"} {
      set mm(work) "";       # clear
      append mm(work) [lindex $mm(tape) $mm(cell)]; # get
      if { $mm(cell) >= $mm(size) } { MoreTape; }
      incr mm(cell);                             # ++
      if {$mm(work) eq [lindex $mm(tape) $mm(cell)]} {
        append mm(work) [lindex $mm(tape) $mm(cell)]; # get
        if { $mm(cell) > 0 } { incr mm(cell) -1 };  # --
        lset mm(tape) $mm(cell) $mm(work);  # put 
        set mm(work) "";       # clear
        append mm(work) "\[palindrome found!\] ending at line "
      ...
      set mm(work) "";       # clear
      if { $mm(cell) > 0 } { incr mm(cell) -1 };  # --
      append mm(work) "pal*pal*"
    }
    if {$mm(work) eq "char*char*"} {
      ...
      set mm(work) "";       # clear
      if { $mm(cell) > 0 } { incr mm(cell) -1 };  # --
      append mm(work) "char*char*"
    }
    if {$mm(work) eq "list*char*"} {
      ...
      if { $mm(cell) > 0 } { incr mm(cell) -1 };  # --
      set mm(work) "";       # clear
      append mm(work) "list*char*"
    }
    #------------
    # 3 tokens
    Pop;
    if {$mm(work) eq "list*list*list*" || $mm(work) eq "char*list*char*" || $mm(work) eq "list*char*list*" || $mm(work) eq "list*pal*list*" || $mm(work) eq "pal*list*pal*" || $mm(work) eq "char*char*char*" || $mm(work) eq "char*pal*char*" || $mm(work) eq "pal*pal*pal*" || $mm(work) eq "pal*char*pal*"} {
      Push;
      Push;
      Push;
      if { $mm(cell) > 0 } { incr mm(cell) -1 };  # --
    ...
    if {$mm(eof)} {
      ...
      if {$mm(work) eq "pal*"} {
        set mm(work) "";       # clear
        append mm(work) "\[whole string is a complete palindrome!\]\n"
        puts -nonewline $mm(work);    # print
        exit;
      }
      if {$mm(work) eq "list*"} {
        set mm(work) "";       # clear
        append mm(work) "\[a list\]\n"
        puts -nonewline $mm(work);    # print
        exit;
      }
      Push;
      Push;
      append mm(work) "\[Whole string isn't a palindrome\]\n"
