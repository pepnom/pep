  ...
  while true {  
    ...
    if {[string match {array**} $mm(work)] || [string match {object**} $mm(work)]} {
      if {$mm(work) ne "array*" && $mm(work) ne "object*" && ![string match *,* $mm(work)]  && ![string match *\}* $mm(work)]  && ![string match *\]* $mm(work)] } {
        set mm(work) "";   # clear
        append mm(work) "Json error near line "
        append mm(work) $mm(linesRead); # lines 
        append mm(work) " char "
        append mm(work) $mm(charsRead); # chars 
        append mm(work) ")\n"
    ...
    if {$mm(work) eq "\[*decimal*" || $mm(work) eq "\[*number*" || $mm(work) eq "\[*string*" || $mm(work) eq "\[*value*" || $mm(work) eq "\[*array*" || $mm(work) eq "\[*object*"} {
      set mm(work) "";   # clear
    ...
    if {$mm(work) eq "sign.integer*.*integer*" || $mm(work) eq "integer*.*integer*"} {
      set mm(work) "";   # clear
    ...
    # 
    if {$mm(work) eq "items*,*decimal*" || $mm(work) eq "items*,*string*" || $mm(work) eq "items*,*value*" || $mm(work) eq "items*,*array*" || $mm(work) eq "items*,*object*"} {
      set mm(work) "";   # clear
      append mm(work) "items*"
      Push;
      continue;   # .reparse 
    ...
    # object members
    #"string*:*integer*",
    if {$mm(work) eq "string*:*decimal*" || $mm(work) eq "string*:*string*" || $mm(work) eq "string*:*value*" || $mm(work) eq "string*:*object*" || $mm(work) eq "string*:*array*"} {
      set mm(work) "";   # clear
    ...
    if {$mm(eof)} {
      while [Pop] {};           # unstack 
      if {$mm(work) eq "object*" || $mm(work) eq "array*" || $mm(work) eq "value*" || $mm(work) eq "string*" || $mm(work) eq "integer*"} {
        while [Push] {};          # stack 
      ...
      while [Push] {};          # stack 
      append mm(work) "Invalid JSON \n"
      append mm(work) "Caveat: this script currently only recognises signed decimal \n"
      append mm(work) "in the form -22.123 but not scientific numbers in the form \n"
      append mm(work) "-6.11E+10 .\n"
