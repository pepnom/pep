...
while !$mm(eof) { 
  ...
  while true {  
    ...
    if {$mm(work) ne "0"} {
      set mm(work) "";       # clear
      append mm(work) "Unmatched bracket near line "
    ...
    Pop;
    Pop;
    if {$mm(work) eq "\{*\}*" || $mm(work) eq "(*)*" || $mm(work) eq "\[*\]*" || $mm(work) eq "ob*ob*"} {
      set mm(work) "";       # clear
      append mm(work) "ob*"
      Push;
      continue;   # .reparse 
    ...
    Pop;
    if {$mm(work) eq "\{*ob*\}*" || $mm(work) eq "(*ob*)*" || $mm(work) eq "\[*ob*\]*"} {
      set mm(work) "";       # clear
      append mm(work) "ob*"
      Push;
      continue;   # .reparse 
    ...
    if {$mm(eof)} {
      if {$mm(work) eq "ob*"} {
        set mm(work) "";       # clear
        append mm(work) "brackets look balanced!\n"
        puts -nonewline $mm(work);    # print
        exit;
      }
      set mm(work) "";       # clear
      append mm(work) "brackets not balanced.\n"
      puts -nonewline $mm(work);    # print
      exit;
    }
    Push;
