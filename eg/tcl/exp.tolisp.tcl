...
while !$mm(eof) { 
  ...
  while true { 
    ...
    append mm(work) " << incorrect character (at character "
    append mm(work) $mm(charsRead); # chars 
    append mm(work) " of input). \n"
  ...
  while true {  
    ...
    if {$mm(work) eq "number*" || $mm(work) eq "variable*"} {
      set mm(work) "";       # clear
      append mm(work) " "
    ...
    # 4 tokens
    Pop;
    if {$mm(work) eq "exp*opadd*exp*opadd*"} {
      set mm(work) "";       # clear
      append mm(work) " ("
      if { $mm(cell) >= $mm(size) } { MoreTape; }
      incr mm(cell);                             # ++
    ...
    if {$mm(eof)} {
      ...
      Push;
      Push;
      append mm(work) "No, it doesn't look like a valid 'in-fix' expression. \n"
