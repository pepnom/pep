...
while !$mm(eof) { 
  ...
  while true { 
    ...
    if {[regexp {^[ \t\f]+$} $mm(work)]} {
      # while  
      while {[regexp {^[ \t\f]+$} $mm(peep)]} { if {$mm(eof)} { break; } Read }
      set mm(work) "";       # clear
      if {$mm(eof)} {
        break;  # .reparse 
      }
  ...
  while true {  
    ...
    if {$mm(work) eq "word*word*" || $mm(work) eq "text*word*"} {
      set mm(work) "";       # clear
    ...
    if {$mm(work) eq "word*nl*" || $mm(work) eq "text*nl*"} {
      set mm(work) "";       # clear
      append mm(work) [lindex $mm(tape) $mm(cell)]; # get
      if { $mm(cell) >= $mm(size) } { MoreTape; }
      incr mm(cell);                             # ++
    ...
    if {$mm(work) eq "line*line*" || $mm(work) eq "lineset*line*"} {
      set mm(work) "";       # clear
      append mm(work) [lindex $mm(tape) $mm(cell)]; # get
      if { $mm(cell) >= $mm(size) } { MoreTape; }
      incr mm(cell);                             # ++
    ...
    if {$mm(eof)} {
      Pop;
      if {$mm(work) eq "word*" || $mm(work) eq "text*" || $mm(work) eq "line*" || $mm(work) eq "lineset*"} {
        set mm(work) "";       # clear
        append mm(work) [lindex $mm(tape) $mm(cell)]; # get
        append mm(work) "\n"
        puts -nonewline $mm(work);    # print
