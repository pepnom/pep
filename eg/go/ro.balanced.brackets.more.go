...
func main() {
  ...
  for !mm.eof { 
    ...
    for true {
      ...
      if (mm.work != "0") {
        mm.work = ""          // clear
        mm.work += "Unmatched bracket near line "
      ...
      mm.pop();
      mm.pop();
      if (mm.work == "{*}*" || mm.work == "(*)*" || mm.work == "[*]*" || mm.work == "ob*ob*") {
        mm.work = ""          // clear
        mm.work += "ob*"
        mm.push();
        continue
      ...
      if (strings.HasPrefix(mm.work, "{*")) {
        if (strings.HasSuffix(mm.work, "]*") || strings.HasSuffix(mm.work, ")*")) {
      ...
      mm.pop();
      if (mm.work == "{*ob*}*" || mm.work == "(*ob*)*" || mm.work == "[*ob*]*") {
        mm.work = ""          // clear
        mm.work += "ob*"
        mm.push();
        continue
      ...
      if (strings.HasPrefix(mm.work, "{*ob*")) {
        if (strings.HasSuffix(mm.work, "]*") || strings.HasSuffix(mm.work, ")*")) {
          mm.tape[mm.cell] = mm.work  /* put */
          mm.work = ""          // clear
          mm.counter++    /* a+ */
      ...
      if (mm.eof) {
        if (mm.work == "ob*") {
          mm.work = ""          // clear
          mm.work += "brackets look balanced!\n"
          fmt.Printf("%s", mm.work)    // print
          os.Exit(0)
        }
        mm.work = ""          // clear
        mm.work += "brackets not balanced.\n"
        fmt.Printf("%s", mm.work)    // print
        os.Exit(0)
      }
      mm.push();
