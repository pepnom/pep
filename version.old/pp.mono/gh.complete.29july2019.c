...
FUTURE DIRECTIONS 
  ...
  gave some thought to how to reorganise this, now that the machine 
  is getting to a really useful stage. Here are ideas:
   ...
   * reorganise the code files into:
        machine.c - just the machine structure and methods (program, etc)
        machine.loop.c - the interactive prog (included in gh.c at the moment)
        machine.prog - 
           this will be the actual unix style stream filter, and will
           not include any interactive testing code.
        machine.methods.c - 
           these are just the parts of execute() rewritten as functions,
           one for each machine command. The point of this, is that it
           allows us to compile scripts!!
        machine.execute - 
           which just contains the execute() function, because this is 
           not required when compiled scripts.

   * allow scripts to be compiled! We do this by writing a script, which
     will convert any script into a series of c "method" calls on the
     machine, and then compile that program.
...
COMPILATION OF SCRIPTS
   ...
   script happens as follows. The machine loads an assembly-code file called
   "asm.pp". It then uses that script to compile a given script into the
   equivalent assembly-code (and saves the code in a text file called
   something like "script.pp" Then it loads that assembly-code file and uses
   it to process the input. 
   ...
   This made hand-coding of assembly programs feasible and possible. But
   it is still easier to use the script language instead.
...
BUGS 
    ...
    * until not working in some cases!. well until and while have 
      no malloc!! so that could be one reason, they dont grow the 
      buffer as necessary. NOT true, buffer should grow in readc,
      but growing it in "until" seemed to fix the bug. So need to 
      keep looking. Well growing it in "until" seems to work, even 
      though it should grow in readc but doesnt.
      This bug needs to be dealt with.
...
HISTORY
    ...
    languages may be very interesting. We shall see. The system may  now 
    be useful (2019)

    The idea of a parsing machine derived from thinking about the sed stream
    editor and its limitations, bnf grammars, context-free languages, human
    languages, the limitations of regular expressions and regular languages.
    At the very least this virtual machine should be able to handle nested
    structures which "sed" is unable to handle.  So lisp-like syntax (+ (* n
    n) m) should be easily parsable.
    ...
    28 july 2019
      ...
      echo "abcd" | pp -f palindrome.pss  eventually)
    ...
    27 july 2019, in bogota, Colombia
      ...
      wrote a simple palindrome detecter. It could be made more 
      elegant. wrote quotesets with a few tricks that seemed to
      work. So multiple tests for one block should be possible
      "quotesets" are implemented in asm.pp and resolve into 
      tests for blocks. They are very useful because they allow
      syntax like this:
        "noun*verb*object*", "article*verb*object*" {
           # ...
        }
    ...
    26 july 2019
      ...
      workspace buffer, leading to bugs. Fixed the until instruction
      but need to do "while" as well.
    ...
    25 july 2019
     ...
     the <pp> parse label. Finished end and begins test and blocks.
...
/* replace one substring with other text in a string. store the result
   in result. The result buffer needs to be big enough other wise
...
// NEWLINE,/* adds a newline to the  workspace, just an alias
//    for "add '\n';"
// reparse (jumps back to parse label, but this is
//  or can be implemented in asm.pp, this is necessary.j
...
enum TestCommand { 
  ...
  STEPCODE, RUNCODE, RUNZERO, RUNCHARSLESSTHAN, RUNTOTRUE, 
...
struct {
  ...
  { COMPILEMODE, {"m.c", ""}, "", 
     "compile mode: entered instructions are compiled but "
     "not executed" },
  { IPCOMPILEMODE, {"m.ipc", ""}, "", 
...
int readc(struct Machine * mm) {
   ...
   if (strlen(mm->buffer.stack) == (mm->buffer.capacity - 1)) 
...
enum ExitCode execute(struct Machine * mm, struct Instruction * ii) {
  ...
  switch (ii->command) {
    ...
    case CLEAR:
      ...
      break;
    case REPLACE:
      difference = strlen(ii->b.text) - strlen(ii->a.text);
    ...
    case UNTIL:
      ...
      // this should not be necessary because buffer grows in 
      // readc !!!!!
      if (strlen(mm->buffer.stack) == mm->buffer.capacity - 1) {
...
/* another debugging tool: run while the number of characters read
   by the machine is less than maximum */ 
