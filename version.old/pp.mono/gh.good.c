...
int main (int argc, char *argv[]) {
  ...
  */
  ...
  if (scriptFileName != NULL) {
    ...
    strcpy(new.program.source, "asm.pp"); 
    loadAssembledProgram(&new.program, asmFile);
    ...
    if (progmode != FILTERMODE) {
      printf("\n");
      printf("Loading script file %s'sav.pp'%s \n", YELLOW, NORMAL);
    }
    loadAssembledProgram(&m.program, savFile);
  }

  if ((progmode == FILTERMODE) &&
     ((scriptFileName != NULL) || (asmFileName != NULL))) {
