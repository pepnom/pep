...
HISTORY
    ...
    (about 15 as of 2018) with many stops and starts in the meantime 
    as well as many deadends. In fact I dont even know if the idea
    in itself is sound. 

    Nevertheless I have a hunch that this approach to parsing
    context-free languages may be very interesting.  We shall see. The code is
    quite close to being useful (2018)
...
/* just build an array with label line numbers
   void buildTable(struct JumpTable * table, FILE * file) {
*/
void buildJumpTable(struct Label table[], FILE * file) {
  char buffer[1000];
  char * line;
  int lineNumber;
  char text[2001];
...
void loadAssembledProgram(struct Program * program, FILE * file) {
  ...
  switch (ii->command) {
    ...
    case WRITE:
      ...
      if ((saveFile = fopen("sav.pp", "w")) == NULL) {
        printf ("Cannot open file %s'sav.pp'%s \n", YELLOW, NORMAL);
        return 4;
      }
...
void printUsageHelp() {
  printf("'pp' a pattern parsing machine \n");
...
int main (int argc, char *argv[]) {
  ...
  int c;
  // a file containing script-assembler to be compiled and run
  char * asmFile = NULL;
  char * inputFile = NULL;
  ...
  enum { FILTERMODE, INTERMODE } progmode = INTERMODE;

  opterr = 0;

  while ((c = getopt (argc, argv, "i:f:Isha:")) != -1) {
    switch (c) {
      ...
      case '?':
        if (optopt == 'a') {
          ...
          fprintf (stderr, "%s: option -%c requires an argument.\n", 
             argv[0], optopt);
          fprintf (stderr, " -i scriptFile \n");
        } else if (isprint (optopt)) {
          fprintf (stderr, "%s: unknown option -- %c \n", argv[0], optopt);
          printUsageHelp(); 
        } else {
          printUsageHelp(); 
          fprintf (stderr, "%s: unknown option char -- '\\x%x' ", argv[0], optopt);
        }
        return 1;
      default:
        abort ();
  ...
  */
  ...
  if ((asmFile != NULL) && (scriptFile != NULL)) {
  ...
  if (asmFile != NULL) {
    ...
    loadAssembledProgram(&m.program, fp);
    fclose(fp);
  }

  if ((progmode == FILTERMODE) && (asmFile != NULL)) {
   ...
   */
    ...
    // ENTER key pressed ....
      ...
      loadAssembledProgram(&m.program, loadfile);
