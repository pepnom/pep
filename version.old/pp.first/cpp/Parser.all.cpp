  ...
  bool Parser::parse(istream& inputstream)
  {
    ...
    {             
       ...
       if (this->mm.workspace() == "/")
       {
         ...
         this->mm.clip();
         this->mm.replace("\"", "\\\"");
         this->program.addInstruction("if-is", this->mm.workspace());
         /* the if jump offset has to be reset for each new test */
         iJump = 0;
       ...
       if (this->mm.workspace() == "<")
       {
         ...
         this->mm.clip();
              /* escape double quotes */
         this->mm.replace("\"", "\\\"");
         this->program.addInstruction("if-begins-with", this->mm.workspace());
         /* the if jump offset has to be reset for each new test */
         iJump = 0;
       ...
       if ((this->mm.workspace() == "test|test|"))
       {
         ...
         this->mm.get();
         this->mm.decrementTape();
         this->program.addParameter(this->mm.workspace());
         this->mm.clear();
