...
HISTORY
  ...
  16 June 2021
    ...
    tried to fix a bug in until which didnt like eg: add "\\" or add "\\\"ab"
    Also, a gotcha!!
    ...
    */
    ...
    case UNTIL:
      ...
      */
      ...
      // already ends with the given text (that is just its behaviour to
      // make it useful).
      ...
      // after just one char, maybe we dont have to worry about the 
      // situation with \\\\\\suffix etc, but later we will
      ...
      while (readc(mm)) {
        ...
        if (strcmp(suffix, ii->a.text) == 0) {
          ...
          nn = 2;
          while (((suffix-nn) >= mm->buffer.workspace) && (*(suffix-nn) == mm->escape)) {
            nn++;
          }
