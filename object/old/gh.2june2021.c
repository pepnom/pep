...
/*
  ...
  http://bumble.sourceforge.net/books/pars/object/gh.c

OVERVIEW

  This file and the other files in this folder /books/gh/object/ implements
  a virtual machine which is particularly apt for parsing context free
  languages. Along with the files /books/gh/asm.pp and /books/gh/compile.pp
 ...
 * Currently the machine is using standard c chars which is clearly
   ...
   and then use a script like compilable.c.pss to compile scripts into
...
TASKS
   ...
   * split pp into 2 (just an interpreter that can get input from
   ...
   * write a man page. generate the manpage option help from the 
     pp -h command. Look at asciidoc conversion to man page

   * write a sed script to convert comments in gh.c to asciidoc
...
COMPILING AND RUNNING THIS CODE
  ...
  * compile code
  >> see the functions in helpers.gh.sh 
...
SEE ALSO
   ...
   compilable.c.pss
     ...
     helpers.gh.sh for the exact gcc compiling code. 
...
HISTORY
    ...
    23 august 2019
      ...
      * create a javascript script parser/compiler 
      >> pp -f convert.javascript.pss convert.javascript.pss > pp.js
    ...
    20 august 2019: sunny
      ...
      Writing a man page for pp. But I will use asciidoc and convert
    ...
    14 august 2019: Bogota, Colombia - raining
      ...
      not to asm.pp . So now we can do 
       >> pp -f compile.pss -i ' !""{a"not empty!!";}t;d;'
      ...
      script compilable.c.pss (see the "ghcl" helper bash function)
      ...
      programs from a script with
        pp -f compile.ccode.pss script > script.c
        gcc -o scriptx script.c -Lobject/ -lmachine

    13 august 2019

      Continued to separate the code in gh.c into separate 'object' 
      files in the gh/object/ folder. Currently up to machine.c 
      now will do machine.interp.c  The code is compiling with the bash 
      function ppco which is in the file gh/helpers.gh.sh . I am
      not using 'make' to compile, currently.

    12 august 2019

      Reorganising the source code files. The main c file is now
      gh/object/gh.c and this includes the other 'object' files which
      are in this directory. Moved the old gh.c source code files 
      to the folder Monolith.gh (because everything was in the one
      file).

      Made the files in the gh/object folder the canonical source code for
...
enum ExitCode loadScript(
   struct Program * program, FILE * scriptFile, int position) {
    ...
    if ((asmFile = fopen("asm.pp", "r")) == NULL) {
      printf("loadScript(): could not open assembler %sasm.pp%s ...\n", 
        YELLOW, NORMAL);
      exit(1);
    } 
    FILE * savFile;
