...
int readc(struct Machine * mm) {
   if (mm->peep == EOF) return 1;
   if (feof(mm->inputstream)) return 2;
...
int main (int argc, char *argv[]) {
  ...
  char source[64] = "input.txt";
    ...
    fflush(stdout);
    // just checking stdin and stdoutjfk
    printf("stdin!");
    while (!feof(stdin)) {
  ...
  */
  ...
  if (scriptFileName != NULL) {
    ...
    loadAssembledProgram(&m.program, asmFile);
    //m.inputstream = scriptFile;
    // this run is crashing the system. no feof() check?
    run(&m);
    //runDebug(&m);
    fclose(asmFile);
    // now open sav.pp as "asmFile" and load it.
    fclose(scriptFile);
  ...
  if ((progmode == FILTERMODE) && (asmFileName != NULL)) {
